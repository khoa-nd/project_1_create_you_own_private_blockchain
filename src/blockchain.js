...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - messageTime < 300) {
                const verifiedMessage = bitcoinMessage.verify(message, address, signature);
                if (verifiedMessage) {
                    this.validateChain();
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            const blockByHash = self.chain.filter(block => block.hash === hash);
            if (blockByHash && blockByHash.length > 0) {
                resolve(blockByHash[0]);
            } else {
                resolve({});
            }
        });
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(async block => {
                ...
                if (blockData.owner === address) {
                    stars.push({owner: blockData.owner, star: blockData.star});
                }
            });
            resolve(stars);
        });
